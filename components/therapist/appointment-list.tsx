...
export function AppointmentList({ selectedDate }: AppointmentListProps) {
  ...
  const filteredAppointments = appointments
    .filter((appointment) => {
      // Date filter
      const matchesDate = !selectedDate || isSameDay(new Date(appointment.date), selectedDate)
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Avatar className="h-8 w-8">
                            <AvatarImage src={appointment.patientAvatar || patient?.avatar} />
                            <AvatarFallback>{appointment.patientName.charAt(0)}</AvatarFallback>
                          </Avatar>
                          <span className="font-medium">{appointment.patientName}</span>
                        </div>
                      </TableCell>
                      <TableCell>{format(new Date(appointment.date), "MMM d, yyyy 'at' h:mm a")}</TableCell>
